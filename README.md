...
Every archive unpacks to a single `Proteus/` folder holding the program, a start
script beside it, and the checksum that script checks:

| | Windows | macOS | Linux |
|---|---|---|---|
| the program | `Proteus.exe` | `Proteus` | `Proteus` |
| start it with | `start.cmd` | `start.command` | `start.sh` |
| what that checks | `Proteus.exe.sha256` | `Proteus.sha256` | `Proteus.sha256` |
...
first. `PROTEUS_SKIP_VERIFY=1` turns the check off for anyone who has changed
...
and identifies a legal entity; Proteus is one person's project given away under the AGPL,
...
Right-click it and choose **Open**, or run `xattr -dr com.apple.quarantine Proteus`.
...
Those are the digests worth checking, and the `Proteus.sha256` inside the archive is not a
