...
to accompany the object code, and Proteus's own AGPL requires the same.
Shipping a hundred-odd libraries with none of their terms attached is a
straightforward compliance defect, and THIRD-PARTY-LICENSES.md sitting in the
repository does not fix it — somebody who downloads a zip never sees it.

Proteus is built with ``--onefile``, so unlike Orion there is no bundle
directory to add these to: putting them in the bundle would seal them inside
the executable, where they would be visible only to someone who had already
run it. The release workflow therefore runs this script and packages the tree
it produces *beside* the binary.
...
#: interpreter frozen into the executable, and the toolkit tkinter binds to.
...
#: PyInstaller is deliberately *not* among them: its bootloader is the first
#: thing in the executable, under an exception that permits exactly that, so
...
def collect(repo: str, staging: str) -> str:
    ...
    index = [
        ...
        "",
        "Proteus itself is licensed AGPL-3.0-or-later; the full text is in",
        "`Proteus-LICENSE.txt`. A commercial licence, without the AGPL's",
        ...
        "## Proteus",
        "",
        "- `Proteus-LICENSE.txt` — GNU Affero General Public License v3.0",
        "",
    ]

    shutil.copyfile(
        os.path.join(repo, "LICENSE"), os.path.join(staging, "Proteus-LICENSE.txt")
    )
    ...
    index += [
        ...
        "linked dynamically. Every such library is a separate file inside this",
        "executable's archive, which PyInstaller unpacks to a directory at",
        "startup, so a recipient can replace one with a modified build of the",
        "same library. `system/` holds their terms; the AGPL text in",
        "`Proteus-LICENSE.txt` and the sources in the repository cover",
        "Proteus's own code.",
        "",
    ]
